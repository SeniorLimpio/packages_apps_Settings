...
public class ShakeEvents extends SettingsPreferenceFragment
    ...
    private static final int DLG_LDROID_ACTIONS = 0;
    ...
    private String returnFriendlyName(int setting) {
        ...
        if (uri != null) {
            if (uri.startsWith("**")) {
                ...
                String ldroidDescription = null;
                for (int i = 0; i < dialogValues.length; i++) {
                    if (uri.equals(dialogValues[i])) {
                        ldroidDescription = dialogEntries[i];
                    }
                }
                return ldroidDescription;
            } else {
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mShakeX) {
            ...
            showDialogInner(DLG_LDROID_ACTIONS);
            return true;
